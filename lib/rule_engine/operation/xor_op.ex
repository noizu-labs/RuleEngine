...
defimpl Noizu.RuleEngine.ScriptProtocol, for: Noizu.RuleEngine.Op.XorOp do
        ...
        if Enum.member?([true, :required], this.settings[:throw_on_timeout?]) do
          ...
          outcome = this.arguments
                    ...
                    |> Enum.reduce([],
                         fn({task, res}, acc) ->
                           case res do
                             {:ok, {o, _s}} -> (o && length(acc) < 2) && (acc ++ [o]) || acc
                             _ ->
                               Task.shutdown(task, yield_wait)
                               acc
                           end
  ...
  # render/3
  #---------------------
  def render(this, state, context), do: Helper.render_arg_list("[XOR]", identifier(this, state, context), this.arguments || [], state, context, %{})

  #---------------------
  # render/4
  #---------------------
  def render(this, state, context, options), do: Helper.render_arg_list("[XOR]", identifier(this, state, context, options), this.arguments || [], state, context, options)
end