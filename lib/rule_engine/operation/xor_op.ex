...
defimpl Noizu.RuleEngine.ScriptProtocol, for: Noizu.RuleEngine.Op.XorOp do
  ...
  # render/3
  #---------------------
  def render(node, state, context), do: Helper.render_arg_list("[XOR]", identifier(node), node.arguments || [], state, context, %{})

  #---------------------
  # render/4
  #---------------------
  def render(node, state, context, options), do: Helper.render_arg_list("[XOR]", identifier(node), node.arguments || [], state, context, options)
end