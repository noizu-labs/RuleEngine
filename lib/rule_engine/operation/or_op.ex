...
defimpl Noizu.RuleEngine.ScriptProtocol, for: Noizu.RuleEngine.Op.OrOp do
  ...
  # render/3
  #---------------------
  def render(node, state, context), do: Helper.render_arg_list("[OR]", identifier(node), node.arguments || [], state, context, %{})

  #---------------------
  # render/4
  #---------------------
  def render(node, state, context, options), do: Helper.render_arg_list("[OR]", identifier(node), node.arguments || [], state, context, options)
end